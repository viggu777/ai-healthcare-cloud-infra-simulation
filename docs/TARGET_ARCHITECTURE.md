...
- **Nginx upstream DNS — measured, partially remediated:** `--scale api=2` sent 60/60 test requests to one replica at startup-pin (`docs/RESILIENCE.md` §4). Since `c744877` the gateway re-resolves `api` every 10 s (`resolver` + variable `proxy_pass`) with 502/503 failover, so rescheduling no longer needs a reload — but per-request round-robin is still absent (re-test: 44/0 across two batches, window-pinned). Full remediation = real ingress; until then scale workers, not the API.
...
| Logging, metrics, health checks, dashboard (§16) | `/health`,`/ready`,`/metrics` + `/metrics/prom`, Prometheus, Grafana | Text exposition endpoints + 3 exporters + provisioned 19-panel dashboard (incl. AI-inference + EHR-mock) | 1 (endpoints) / 4 (done: scraped, visualized) | Dashboard live on 127.0.0.1:3000/3001 |
