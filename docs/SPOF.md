...
| **Worker** (1 replica default) | Stop/crash → backlog grows | Latency of processing, not loss (Redis holds jobs) | `restart: on-failure`; WorkerDown + QueueBacklog alerts; linear `--scale worker=N` (measured 2.2×) | Detection is built; recovery still needs an operator (or a future autoscaler) |
...
| **Deployment system** (pipeline.sh / Actions) | Bad gate or bad tag | Blocked releases (safe) or rolled-back deploy (safe) | Every post-build failure restores the previous tag; traceability table per run | No git remote here → SHA tagging falls back to timestamped run tags locally |
