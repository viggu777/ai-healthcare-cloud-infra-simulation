...
- **R4 — Placeholder credentials committed (`dev.env`/`prod.env`).** Non-production values, Gitleaks-clean. Rule: inject real secrets via host env/secret store before any shared use.
